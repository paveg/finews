...
- significance は「指定されたセクターの投資家にとっての重要度」で判定する
  1=セクターと無関係 or 些末, 2=間接的に関連, 3=注目すべき材料, 4=重要材料, 5=市場を動かす最重要材料
  セクターと無関係な記事は必ず significance=1 とすること
...
export const stage1UserPrompt = (title: string, body: string, sector?: string) =>
  `${sector ? `対象セクター: ${sector}\n\n` : ''}<article>
<title>${title}</title>
<body>${body}</body>
</article>
...
ウォッチリスト速報と市場背景はシステムが自動付加するため、出力しないこと。
